...
from split_audio_file import splitAudio
from time_calculate import time_format
from split_large_audio import split_large_audio_file
...
def get_time_input():
    ...
    while True:
        try:
            print("請依順序輸入在精華影片中要查詢的開始時間（分鐘:秒）：")
            ...
            print("請依順序輸入在精華影片中要查詢的結束時間（分鐘:秒）：")
...
def process_audio(short_voice_url, long_voice_url, start_time, end_time):
    ...
    try:
        ...
        split_large_audio_file.split_audio_ffmpeg(long_voice_path, split_duration, "./segment/segments")
        ...
        while spilt_segment_index_start<spilt_segment_index_end:
            ...
            while current_start < spilt_long_voice_time:
                ...
                print(f"查詢時間段(秒)：{time_format.sec_to_time(spilt_segment_index_start*split_duration+seg_start)} ~ {time_format.sec_to_time(spilt_segment_index_start*split_duration+seg_end)} ")

                # Split audio
                spilt_long_audio_array=splitAudio.split_audio(long_audio_array, seg_start, seg_end, 16000)
